...
from analysis_3pt import meson_3pt_projections_big
from analysis_3pt import meson_2pt_projections
...
#     # dirpart_u = "slrc/kp121095kp121095/"
#     # dirpart_s = "slrc/kp121095kp120512/"
#     # dirpart_s = "slrc/kp120512kp121095/"
...
def meson_gevp_twist():
    ...
    fit_correlator(
        ...
        plotdir,
        datadir,
        name="3pt_ratio1_kpi_u-twist_fwd_fit",
        ylabel=r"$R_1(t)$",
        ylim=(0.29, 0.32),
    ...
    fit_correlator(
        ...
        plotdir,
        datadir,
        name="3pt_ratio1_kpi_s-twist_fwd_fit",
        ylabel=r"$R_1(t)$",
        ylim=(0.29, 0.32),
        time_limits=np.array([[[11, 35], [13, 35]]]),
    )
    ...
    # # ----------------------------------------------------------------------
    # # Quad ratio 2 with all four 3-point functions
    # denominator = np.abs(k_k_fwd[:, :tmax] * p_p_fwd[:, :tmax]) ** (-1)
    # numerator = np.abs(p_k_fwd[:, :tmax] * k_p_fwd[:, :tmax])
    # bsdata_7a = np.sqrt(np.einsum("ij,ij->ij", numerator, denominator))
...
def meson_gevp_big():
    ...
    # pion_fwd = Gt_0twist[0]
    # kaon_fwd = Gt_0twist[1]
    # pion_bwd = Gt_0twist[2]
    # kaon_bwd = Gt_0twist[3]
...
def meson_gevp_big_b5p50kp121040kp120620():
    """
    Read the meson 3pt functions and 2pt functions. Construct a ratio to fit and plot
    For 3pt correlators at qsq_max
    Making ratios which should equal the renormalised matrix element
