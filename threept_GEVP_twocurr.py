...
def fit_correlator(
    ...
    # axs.axvline(54, color="k", linewidth=1, linestyle="--")
...
def meson_2pt_projections(
    ...
    return [evec_left_pion, evec_right_pion, evec_left_kaon, evec_right_kaon], [
        Gt1_pion,
        Gt2_pion,
        Gt1_kaon,
        Gt2_kaon,
...
def meson_3pt_projections(
    ...
    p_k_fwd = np.einsum(
        ...
        # evecs_kaon[1][:, 0],
        "i,ijkl,j->kl",
        evecs_kaon[0][:, evec_num],
        corr_matrix_p_k,
        evecs_pion[1][:, evec_num],
    )
    ...
    k_p_fwd = np.einsum(
        ...
        # evecs_pion[1][:, 0],
        "i,ijkl,j->kl",
        evecs_pion[0][:, evec_num],
        corr_matrix_k_p,
        evecs_kaon[1][:, evec_num],
    )
    # ------------------------------------------------------------
    # pion to pion
...
def meson_gevp_qsqmax_twocurr():
    ...
    plotdir = Path("./plots/qsq_max_twocurr/")
    plotdir.mkdir(parents=True, exist_ok=True)
    datadir = Path("./data/qsq_max_twocurr/")
    datadir.mkdir(parents=True, exist_ok=True)

    nboot = 500
    nbin = 1
    conf_num_u = "496"
    pickledir = Path("./threept_0twist_2currents")
    ...
    pion_dir = "meson_qcdsf"
    kaon_dir = "meson_qcdsf"
    evecs_0twist, Gt_0twist = meson_2pt_projections(
        pickledir,
        plotdir,
        datadir,
        pion_dir=pion_dir,
    ...
    # threept_projected = meson_3pt_projections(
    # p_k_fwd, k_p_fwd, p_p_fwd, k_k_fwd = meson_3pt_projections(
    [p_k_fwd, k_p_fwd, p_p_fwd, k_k_fwd], [
        ...
        evecs_0twist[:2],
        evecs_0twist[2:],
        # [evec_left_pion, evec_right_pion],
        # [evec_left_kaon, evec_right_kaon],
        # p_k_dir=threept_dir_kaon,
        # k_p_dir=threept_dir_pion,
        p_k_dir=threept_dir_pion,
        k_p_dir=threept_dir_kaon,
        p_p_dir=threept_dir_pion,
        k_k_dir=threept_dir_kaon,
        label="qsq_max_twocurr",
    )

    [p_k_bwd, k_p_bwd, p_p_bwd, k_k_bwd], [
        p_k_ps_ps,
        k_p_ps_ps,
        p_p_ps_ps,
        k_k_ps_ps,
    ...
    plot_corr(
        quad_ratio_bs,
        plotname="3pt_pi-k_quad-ratio_fwd",
        plotdir=plotdir,
        v_line=10,
    )
    plot_corr(
    ...
    fit_correlator(
        ...
        # ylim=(0.312, 0.33),
        ylim=(0.315, 0.33),
        time_limits=np.array([[[11, 48], [13, 53]]]),
    )

    # ----------------------------------------------------------------------
    # Ratio with all four 3-point functions. Backwards
    denominator = np.abs(k_k_bwd[:, :tmax] * p_p_bwd[:, :tmax]) ** (-1)
    numerator = np.abs(p_k_bwd[:, :tmax] * k_p_bwd[:, :tmax])
    bsdata_7a = np.sqrt(np.einsum("ij,ij->ij", numerator, denominator))
    quad_ratio_bs_bwd = np.einsum("ij,i->ij", bsdata_7a, energy_factor)

    plot_corr(
        quad_ratio_bs_bwd,
        plotname="3pt_pi-k_quad-ratio_bwd",
        plotdir=plotdir,
        v_line=10,
    )
    plot_corr(
        quad_ratio_bs_bwd,
        plotname="3pt_pi-k_quad-ratio_bwd_ylim",
        plotdir=plotdir,
        v_line=10,
        # ylim=(0.1, 0.8),
        ylim=(0.315, 0.33),
    ...
    plot_proj_corr(
        ...
        0,
        plotdir,
        plot_name="_kaon_and_pion_quad_ratio_bwd",
        labels=["GEVP", r"$\gamma_5,\gamma_5$"],
        # ylim=(0.3, 0.35),
        ylim=(0.312, 0.33),
    )

    quad_ratio_bs_bwd_trev = quad_ratio_bs_bwd
    print(np.shape(quad_ratio_bs_bwd[:, :-1:-1]))
    quad_ratio_bs_bwd_trev[:, 1:] = quad_ratio_bs_bwd[:, :0:-1]
    fit_correlator(
        quad_ratio_bs_bwd_trev,
        plotdir,
        name="3pt_quad_kpi_bwd_fit",
        ylabel=r"$R_2(t)$",
        # ylim=(0.3, 0.35),
        # ylim=(0.312, 0.33),
        ylim=(0.315, 0.33),
    ...
    # quad_ratio_avg = 0.5 * (quad_ratio_bs_bwd_trev[:, :-1] + quad_ratio_bs[:, 1:])
    quad_ratio_avg = 0.5 * (quad_ratio_bs_bwd_trev + quad_ratio_bs)
    fit_correlator(
        quad_ratio_avg[:, :50],
        plotdir,
        name="3pt_quad_kpi_avg_fit",
        ylabel=r"$R_2(t)$",
        # ylim=(0.3, 0.35),
        # ylim=(0.312, 0.33),
        ylim=(0.315, 0.33),
        time_limits=np.array([[[11, 48], [13, 48]]]),
    )
