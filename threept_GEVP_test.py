...
from analysis_3pt import meson_3pt_projections
from gevpanalysis.common import normalize_matrices

# from analysis_3pt import meson_2pt_projections_big
...
def sort_by_previous(evals, evecs, prev_evecs, matrix):
    # Normalize columns
    # current = evecs / np.linalg.norm(evecs, axis=0)
    # previous = prev_evecs / np.linalg.norm(prev_evecs, axis=0)
    current = evecs
    previous = prev_evecs

    # overlap[i, j]: similarity between previous i and current j
    overlap = np.abs(previous.conj().T @ matrix @ current)
    # overlap = np.abs(previous.conj().T @ current)

    # Find the permutation maximizing total overlap
    previous_indices, current_indices = linear_sum_assignment(-overlap)

    # Ensure ordering follows prev_evecs columns
    order = current_indices[np.argsort(previous_indices)]

    evals = evals[order]
    evecs = evecs[:, order]

    # Optionally align each eigenvector's complex phase with its predecessor
    for i in range(evecs.shape[1]):
        phase = np.vdot(prev_evecs[:, i], evecs[:, i])

        if np.abs(phase) > 0:
            evecs[:, i] *= np.exp(-1j * np.angle(phase))
...
def gevp_bootstrap_mesons(
    ...
    for boot in range(nboot):
        ...
        # order = np.argsort(eval_left)
        # eval_left = eval_left[order]
        # evec_left = evec_left[:, order]

        # order = np.argsort(eval_right)
        # eval_right = eval_right[order]
        # evec_right = evec_right[:, order]

        if prev_evecs is not None:
            eval_left, evec_left = sort_by_previous(
                eval_left, evec_left, prev_evecs, mat_0
            )
            # eval_right, evec_right = sort_by_previous(
            #     eval_right, evec_right, prev_evecs
            # )
    ...
    # )
    # Gt3 = np.einsum(
    #     "i,ijkl,j->kl", evec_left_avg[:, 2], corr_matrix, evec_right_avg[:, 2]
    # )

    # if show:
    #     stats.ploteffmass(Gt1, "eig_1" + name, Path("./"), show=False)
    #     stats.ploteffmass(Gt2, "eig_2" + name, Path("./"), show=False)
...
def meson_3pt_projections_big(
    ...
    for iop, op1 in enumerate(opnames):
        for jop, op2 in enumerate(opnames):
            ...
            corr_matrix_p_k[iop, jop] = p_k_complex
            ...
            corr_matrix_k_p[iop, jop] = k_p_complex
...
def meson_gevp_twist():
    ...
    utwist_degen = 1.358140
    theta_range = np.tile(np.linspace(0, 1.5, 30), (500, 1))
    moms = theta_range * np.pi / L
    pion_disp = np.sqrt(pion_energy[:, None] ** 2 + moms**2)
    ...
    plt.plot(
        theta_range[0],
        np.average(pion_disp, axis=0),
        linewidth=1,
        color=_colors[0],
    )
    plt.fill_between(
        theta_range[0],
        np.average(pion_disp, axis=0) - np.std(pion_disp, axis=0),
        np.average(pion_disp, axis=0) + np.std(pion_disp, axis=0),
        alpha=0.4,
        color=_colors[0],
    )
    plt.plot(
        theta_range[0],
        np.average(kaon_disp, axis=0),
        linewidth=1,
        color=_colors[1],
    )
    plt.fill_between(
        theta_range[0],
        np.average(kaon_disp, axis=0) - np.std(kaon_disp, axis=0),
        np.average(kaon_disp, axis=0) + np.std(kaon_disp, axis=0),
        alpha=0.4,
        color=_colors[1],
    )
    ...
    plt.errorbar(
        utwist_degen,
        np.average(pion_energy_degen),
        np.std(pion_energy_degen),
        capsize=4,
        elinewidth=1,
        color=_colors[0],
        fmt="s",
    ...
    plt.errorbar(
        utwist,
        np.average(pion_utwist_energy),
        np.std(pion_utwist_energy),
        capsize=4,
        elinewidth=1,
        color=_colors[0],
        fmt="s",
    ...
    plt.errorbar(
        stwist,
        np.average(kaon_stwist_energy),
        np.std(kaon_stwist_energy),
        capsize=4,
        elinewidth=1,
        color=_colors[1],
        fmt="^",
    ...
    plt.tight_layout()
    plt.savefig(
        plotdir / (f"dispersion_relation.pdf"),
        metadata=_metadata,
    )
    # plt.show()
    plt.close()
...
def ratio1(p_k_3pt, k_p_3pt, pion_2pt, kaon_2pt, energy_factor, norm_factor):
    ...
    bsdata_1 = p_k_3pt * k_p_3pt
    denominator1 = (kaon_2pt * pion_2pt) ** (-1)
    bsdata_1a = np.sqrt(np.abs(np.real(np.einsum("ij,ij->ij", bsdata_1, denominator1))))
    bsdata_dbl = np.einsum(
        "ij,i->ij",
        bsdata_1a,
...
def meson_gevp_twist_jedd():
    ...
    # pion_dir_twist = "meson-2pt_TBC"
    # kaon_dir_twist = "meson-2pt_TBC"
    evals_twist, evecs_twist, Gt_twist, mesons_2pt_twist = meson_2pt_projections(
        pickledir,
        plotdir,
        datadir,
        pion_dir=pion_dir_twist,
    ...
    plot_eff_proj_corrs(
        [kaon_stwist_ps_ps, kaon_stwist_a_a, kaon_stwist_fwd, kaon_stwist_bwd],
        plotdir,
        plot_name="kaon_stwist_GEVP",
        labels=[
            r"$\gamma_5,\gamma_5$",
            r"$\gamma_4\gamma_5,\gamma_4\gamma_5$",
            "fwd",
    ...
    plot_eff_proj_corrs(
        [pion_utwist_ps_ps, pion_utwist_a_a, pion_utwist_fwd, pion_utwist_bwd],
        plotdir,
        plot_name="pion_utwist_GEVP",
        labels=[
            r"$\gamma_5,\gamma_5$",
            r"$\gamma_4\gamma_5,\gamma_4\gamma_5$",
            "fwd",
    ...
    # a_a operators
    # pi -> K
    numerator1 = np.abs(p_k_a_a[:, tau : tau + tmax].real)
    denominator1 = np.abs(kaon_stwist_a_a[:, tau : tau + tmax].real) ** (-1)
    numerator2 = np.sqrt(
        np.abs(pion_a_a[:, :tmax].real * kaon_stwist_a_a[:, tau : tau + tmax].real)
    )
    denominator2 = np.sqrt(
        np.abs(kaon_stwist_a_a[:, :tmax].real * pion_a_a[:, tau : tau + tmax].real)
    ) ** (-1)
    const_factor = (
        np.sqrt(np.abs(kaon_stwist_a_a[:, tau].real / pion_a_a[:, tau].real))
        * energy_factor_stwist
        * norm_factor
    )
    prem_ratio1 = numerator1 * denominator1 * numerator2 * denominator2
    single_ratio_p_k_a_a = np.einsum("ij,i->ij", prem_ratio1, const_factor)

    # ----------------------------------------------------------------------
    # K -> pi
    numerator1 = np.abs(k_p_a_a[:, tau : tau + tmax].real)
    denominator1 = np.abs(pion_a_a[:, tau : tau + tmax].real) ** (-1)
    numerator2 = np.sqrt(
        np.abs(kaon_stwist_a_a[:, :tmax].real * pion_a_a[:, tau : tau + tmax].real)
    )
    denominator2 = np.sqrt(
    ...
    const_factor = (
        np.sqrt(np.abs(pion_a_a[:, tau].real / kaon_stwist_a_a[:, tau].real))
        * energy_factor_stwist
        * norm_factor
    )
    prem_ratio1 = numerator1 * denominator1 * numerator2 * denominator2
    single_ratio_k_p_a_a = np.einsum("ij,i->ij", prem_ratio1, const_factor)
    # ----------------------------------------------------------------------
    ...
    plot_proj_corrs(
        [
            single_ratio_p_k,
            single_ratio_k_p,
            single_ratio_p_k_a_a,
            single_ratio_k_p_a_a,
        ],
        plotdir,
        plot_name="_kaon_and_pion_single_ratio_stwist_ps_ps",
        labels=[
    ...
    # single 3pt ratio with fwd operators
    # k -> k
    numerator1 = k_k_fwd[:, tau : tau + tmax]
    denominator1 = (kaon_stwist_fwd[:, tau : tau + tmax]) ** (-1)
    single_ratio_k_k_fwd = numerator1 * denominator1

    # ----------------------------------------------------------------------
    # ps_ps
    numerator1 = k_k_ps_ps[:, tau : tau + tmax]
    denominator1 = (kaon_stwist_ps_ps[:, tau : tau + tmax]) ** (-1)
    single_ratio_k_k_ps_ps = numerator1 * denominator1

    # ----------------------------------------------------------------------
    # a_a
    numerator1 = k_k_a_a[:, tau : tau + tmax]
    denominator1 = (kaon_stwist_a_a[:, tau : tau + tmax]) ** (-1)
    single_ratio_k_k_a_a = numerator1 * denominator1

    plot_proj_corrs(
        [single_ratio_k_k_ps_ps, single_ratio_k_k_a_a, single_ratio_k_k_fwd],
        plotdir,
        plot_name="_kaonFF_single_3pt_s-twist",
        labels=[r"$\gamma_5,\gamma_5$", r"$\gamma_4\gamma_5,\gamma_4\gamma_5$", "GEVP"],
        ylim=(1.11, 1.18),
        title="s-quark twisted",
        ylabel=r"$R^K_3(t)$",
    ...
    # single 3pt ratio with fwd operators
    # pi -> pi
    numerator1 = p_p_fwd[:, tau : tau + tmax]
    denominator1 = (pion_fwd[:, tau : tau + tmax]) ** (-1)
    single_ratio_p_p_fwd = numerator1 * denominator1

    # ----------------------------------------------------------------------
    numerator1 = p_p_ps_ps[:, tau : tau + tmax]
    denominator1 = (pion_ps_ps[:, tau : tau + tmax]) ** (-1)
    single_ratio_p_p_ps_ps = numerator1 * denominator1

    # ----------------------------------------------------------------------
    numerator1 = p_p_a_a[:, tau : tau + tmax]
    denominator1 = (pion_a_a[:, tau : tau + tmax]) ** (-1)
    single_ratio_p_p_a_a = numerator1 * denominator1

    plot_proj_corrs(
        [single_ratio_p_p_ps_ps, single_ratio_p_p_a_a, single_ratio_p_p_fwd],
        plotdir,
        plot_name="_pionFF_single_3pt_s-twist",
        labels=[r"$\gamma_5,\gamma_5$", r"$\gamma_4\gamma_5,\gamma_4\gamma_5$", "GEVP"],
        ylim=(1.11, 1.18),
        title="s-quark twisted",
        ylabel=r"$R^{\pi}_3(t)$",
    ...
    # ======================================================================
    # At Qsqmax. No momentum
    threept_dir1 = "meson-3pt_SS_SU"
    threept_dir2 = "meson-3pt_UU_US"
    pickledir = Path("./threept_run3")
    ...
    (
        ...
        evecs_0twist[:2],
        evecs_0twist[2:],
        p_k_dir=threept_dir2,
        k_p_dir=threept_dir1,
        p_p_dir=threept_dir2,
        k_k_dir=threept_dir1,
        label="notwist",
        kappa_1="kp121040kp121040",
        kappa_2="kp120620kp121040",
    )
    ...
    # k -> k
    numerator1 = k_k_fwd[:, tau : tau + tmax]
    denominator1 = (kaon_fwd[:, tau : tau + tmax]) ** (-1)
    single_ratio_k_k_fwd = numerator1 * denominator1

    # ----------------------------------------------------------------------
    numerator1 = k_k_ps_ps[:, tau : tau + tmax]
    denominator1 = (kaon_ps_ps[:, tau : tau + tmax]) ** (-1)
    single_ratio_k_k_ps_ps = numerator1 * denominator1

    # ----------------------------------------------------------------------
    numerator1 = k_k_a_a[:, tau : tau + tmax]
    denominator1 = (kaon_a_a[:, tau : tau + tmax]) ** (-1)
    single_ratio_k_k_a_a = numerator1 * denominator1

    plot_proj_corrs(
        [single_ratio_k_k_ps_ps, single_ratio_k_k_a_a, single_ratio_k_k_fwd],
        plotdir,
        plot_name="_kaonFF_single_3pt_notwist",
        labels=[r"$\gamma_5,\gamma_5$", r"$\gamma_4\gamma_5,\gamma_4\gamma_5$", "GEVP"],
        ylim=(1.11, 1.18),
        title="no twists",
        ylabel=r"$R^K_3(t)$",
    ...
    plot_proj_corrs(
        [single_ratio_p_p_ps_ps, single_ratio_p_p_a_a, single_ratio_p_p_fwd],
        plotdir,
        plot_name="_pionFF_single_3pt_notwist",
        labels=[r"$\gamma_5,\gamma_5$", r"$\gamma_4\gamma_5,\gamma_4\gamma_5$", "GEVP"],
        ylim=(1.11, 1.18),
        title="no twists",
        ylabel=r"$R^{\pi}_3(t)$",
...
def meson_gevp_big_twist():
    ...
    plt.style.use(mystyle.as_posix())
    plt.rc("text.latex", preamble=r"\usepackage{physics}")
    plotdir = Path("./plots/twisted_gevp/")
    read_datadir = Path("./data/twisted/")
    datadir = Path("./data/twisted_gevp/")
    ...
    # ----------------------------------------------------------------------
    # Twisted energies
    pion_utwist_file = Path("./plots/twisted/") / (
        f"time_window_loop_pion_u-twist_Cosh.pkl"
    )
    kaon_stwist_file = Path("./plots/twisted/") / (
        f"time_window_loop_kaon_s-twist_Cosh.pkl"
    )
    if pion_utwist_file.is_file():
        with open(pion_utwist_file, "rb") as file_in:
            fitlist_pion_utwist = pickle.load(file_in)
    ...
    evals_0twist, evecs_0twist, Gt_0twist, mesons_2pt = meson_2pt_projections_big(
        pickledir,
        plotdir,
        read_datadir,
        pion_dir=pion_dir,
    ...
    print("evecs = ")
    print(np.shape(evecs_0twist))
    print(evecs_0twist[2][:, 0])
    print(evecs_0twist[2][:, 1])
    print(evecs_0twist[2][:, 2])
    print(evecs_0twist[2][:, 0])
    print(evecs_0twist[2][:, 1])
    print(evecs_0twist[2][:, 2])
    # print(pprint.pprint(evecs_0twist[2]))
    ...
    # time_choice = 3
    time_choice = 3
    delta_t_range = np.arange(-time_choice + 1, 40)
    delta_t_range = delta_t_range[delta_t_range != 0]
    # delta_t_range = np.arange(7, 11)
    prev_evecs = None
    for i_delta, delta_t in enumerate(delta_t_range):
        print(delta_t)
        ...
        # if delta_t == 1:
        if i_delta == 0:
            prev_evecs = evecs_twist[2]
        eval_list.append(evals_twist[2])
    eval_list = np.array(eval_list)

    print(np.shape(eval_list))

    f, axs = plt.subplots(1, 1, figsize=(7, 5), sharex=True, sharey=True)
    energy1 = -(np.log(eval_list[1:, :, 0]) - np.log(eval_list[:-1, :, 0]))
    energy2 = -(np.log(eval_list[1:, :, 1]) - np.log(eval_list[:-1, :, 1]))
    energy3 = -(np.log(eval_list[1:, :, 2]) - np.log(eval_list[:-1, :, 2]))
    print(np.shape(eval_list))
    plt.errorbar(
        delta_t_range[:-1],
        np.average(energy1, axis=1),
        np.std(energy1, axis=1),
        color=_colors[0],
        marker=_markers[0],
        capsize=3,
        ms=5,
    ...
    plt.errorbar(
        delta_t_range[:-1] + 0.1,
        np.average(energy2, axis=1),
        np.std(energy2, axis=1),
        # energy2,
        color=_colors[1],
        marker=_markers[1],
        capsize=3,
    ...
    plt.errorbar(
        delta_t_range[:-1] + 0.2,
        np.average(energy3, axis=1),
        np.std(energy3, axis=1),
        # energy3,
        color=_colors[2],
        marker=_markers[2],
        capsize=3,
    ...
    plt.grid(True, alpha=0.3)
    plt.ylim(-1, 1)
    plt.savefig(plotdir / (f"evals.pdf"))
    plt.show()
    plt.close()
    exit()
...
if __name__ == "__main__":
    ...
    # meson_gevp_qsqmax()
    # meson_gevp_twist()
    # meson_gevp_twist_jedd()
    meson_gevp_big_twist()
